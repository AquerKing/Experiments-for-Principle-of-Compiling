...
struct ProgramRuntimeInfo {
  std::vector<uint32_t> KeywordList;
  std::vector<uint32_t> SeparatorList;
  std::vector<uint32_t> ArithmeticOperatorList;
  std::vector<uint32_t> RelationalOperatorList;
...
int main(int argc, char *argv[]) {
  ...
  // the startup message and exit. If the user has provided arguments, check if
  // any of them match the known command handlers and execute the corresponding
  // handler function.
  if (AllArgs.size() == 1) {
  ...
  // argument (the first argument is the program name) and check if any of them
  // match the known command handlers. If a match is found, execute the
  ...
  for (size_t i = 1; i < AllArgs.size(); ++i) {
    ...
    if (HandlerIt != CommandHandlers.end()) {
      // If a handler is found for the current argument, collect the remaining
      // arguments that are not options (i.e., do not start with '-') and pass
...
void LoadConfiguration(std::vector<std::string_view> &Args) {
  // Check if the user has provided a configuration file path as an argument. If
  // not, print an error message and the usage message, and return.
  if (Args.empty()) {
  ...
  // Read the entire content of the configuration file into a string. If reading
  // fails, print an error message and return.
  std::streamsize Size = ConfigFile.tellg();
  ConfigFile.seekg(0, std::ios::beg);

  std::string Content(Size, '\0');
  if (!ConfigFile.read(Content.data(), Size)) {
  ...
  // RuntimeInfo structure. If parsing fails, print an error message and return.
  ...
  // Convert the relevant fields from the JSON configuration to the appropriate
  // format (e.g., vectors of uint32_t) and store them in the RuntimeInfo
  // structure. If any of the fields are missing or have an invalid format,
  // print an error message and return.
  try {
    RuntimeInfo.KeywordList =
        ConvertStringToU32Vector(ConfigJson.at("keywords").get<std::string>());
    RuntimeInfo.SeparatorList = ConvertStringToU32Vector(
        ConfigJson.at("separators").get<std::string>());
    RuntimeInfo.ArithmeticOperatorList = ConvertStringToU32Vector(
        ConfigJson.at("arithmetic_operators").get<std::string>());
    RuntimeInfo.RelationalOperatorList = ConvertStringToU32Vector(
        ConfigJson.at("relational_operators").get<std::string>());
